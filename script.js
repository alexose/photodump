...
function fitImages() {
    ...
    const blocks = Array.from(images).map(d => ({ w: d.offsetWidth, h: d.offsetHeight, el: d }));
    const result = pckr.fit(blocks);
    blocks.forEach(({ el, fit }) => {
...
const pckr = new Packer(4000, 800);
